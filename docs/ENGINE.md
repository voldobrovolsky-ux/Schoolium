...
> **Легаси-пересмотр 2026-08-30:** имена событий ниже (`edustore.*`) —
> формат, действовавший до канона событий (G-16, AR-23/24: запрет
> `edustore.*`, литерал `<домен>.<агрегат>.<глаголПрош>.v<N>`). Актуальный
> каталог событий — в коде и `docs/ar/events.md`. Остальное содержимое —
> историческая запись решений, принятых при закрытии движка Phase 1.
