# Пилотный auth (AUTH_MODE=pilot-qr) — ВРЕМЕННЫЙ, ЗАМЕНЁН

> **Легаси-пересмотр 2026-08-30:** режим `pilot-qr`, описанный ниже, заменён
> (AR-49 → **AR-94**: контур доступа без SMS, QR-регистрация + привязка
> устройства + одноразовый код с карточки у модератора) и далее — **AR-154**
> (идентичность — юзернейм и пароль). Флёр OIDC, на подключение которого этот
> документ ссылается как на цель, не используется — см. `docs/TENANCY.md`.
> Актуальный контур — `docs/ar/platform.md` (AR-94), `docs/ar/truth.md`
> (AR-154), ворота G-24/G-44/G-64. Ниже — историческая запись решения по
> уже вытесненному режиму.
