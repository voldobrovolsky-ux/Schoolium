...
> **Рабочий канал деплоя действующего стенда — GitHub Actions, не этот раннбук.**
> Прод schoolium.store живёт по [docs/PROD-STATUS.md](PROD-STATUS.md): единственный
> канал изменений — workflow `.github/workflows/deploy-school.yml`
> (`workflow_dispatch`, action = `deploy`/`bootstrap`/`import`/`status`), деплой
> идёт с `main`, после зелёного health-check workflow сам ставит теги
> `deploy/<дата>` и двигает `deploy-current`/`deploy-previous` (откат = запуск
> workflow с предыдущим тегом). Живость проверяется роутом `/api/healthz` —
> не legacy-роутами `/api/auth/flor/*`. Команды ниже — раннбук ручного
> развёртывания нового стенда с нуля, когда workflow ещё не настроен.
> Действие `subjects-merge` (AR-201) сливает регистровые дубли карточек
> предметов школы по `workspace`: без `apply` — dry-run с печатью плана (пары,
> канон, привязки, слоты, уроки, часы, Д6-конфликты), с `apply=true` —
> исполнение транзакцией на пару; сначала всегда dry-run.
...
**Всё состояние версии 1.2.0 — это одна база Postgres.** Контур `schoolium` не
...
Работать 1.2.0 будет и на минимальном тарифе: школа на 300 учеников — это
...
> ⚠️ **Deprecated.** Всё ниже описывает вытесненный контур (Флёрус OIDC, домен
> `edustore-flor-group.ru`, ASR, Keycloak-эпоха) и к действующему стенду
> schoolium.store отношения не имеет. Хранится до вывода контура из кода;
> не использовать как инструкцию.
