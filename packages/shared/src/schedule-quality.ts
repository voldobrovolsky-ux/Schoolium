...
export const INVARIANT_TITLES: Record<ScheduleInvariant, string> = {
  ...
  'I-6': 'уроков в день не больше числа «уроков в день» школы (AR-199: потолков параллели нет)',
  'I-7': 'координаты урока лежат в сетке недели',
  'I-8': 'параметры дня согласованы: длина урока положительна, длина дня конечна (AR-199: норматива 420 минут нет)',
};

/**
 * Инварианты, за которыми стоит внешняя норма. С AR-199 (школа полного дня, пакет
 * 1.5.0) таких нет: I-6 и I-8 держат согласованность с параметрами школы, а не
 * СанПиН. Перечень оставлен пустым, а не удалён — контракт слоя качества не меняется.
 */
export const LEGAL_INVARIANTS: ScheduleInvariant[] = [];
