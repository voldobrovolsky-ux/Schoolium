 ...
 * Schoolium с известной школой, ложатся школьные разрешения `S-62` (AR-212): администратор
...
export class PermissionGuard implements CanActivate {
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    // (AR-212): тумблер `S-62` обязан закрывать роут, а не только прятать кнопку.
    const permissions =
