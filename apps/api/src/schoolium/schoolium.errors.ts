...
const TEXTS: Record<ErrorCode, (d: D) => string> = {
  ...
  LAST_MODERATOR: () => 'Это единственный модератор школы — отозвать активацию или удалить профиль нельзя',
  LAST_ROLE: () => 'Это единственная роль сотрудника — снять её нельзя; чтобы закрыть доступ, отзовите активацию',
  ...
  // выведен из употребления (AR-213): у персонала «Удалить профиль» проходит и
  // при истории; текст хранится, как хранятся тексты кодов СанПиН (AR-199)
  ...
  ADMIN_ACCOUNT_LOCKED: (d) => `Учётной записью администратора школы (${n(d, 'name')}) управляет только администратор`,
...
const STATUS: Partial<Record<ErrorCode, HttpStatus>> = {
  ...
  ADMIN_ACCOUNT_LOCKED: HttpStatus.FORBIDDEN,
