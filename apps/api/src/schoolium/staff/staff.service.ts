...
export class StaffService {
  ...
  async createActivationToken(cardId: string): Promise<ActivationTokenDto> {
    ...
    // Отзыв активации возвращает карточку в «Не авторизованные» (AR-212), и
  ...
  /**
   * `S-31.btn.revokeActivation` — «Отозвать активацию» (AR-212, вытесняет
   ...
   * активации (AR-212). Сессии не воскресают — вход заново по QR либо коду.
  ...
  /**
   * `S-31.btn.deleteStaff` — «Удалить профиль» (AR-212). Вторая из двух
  ...
  /**
   * Физическое стирание профиля (AR-212) — **перечислением, а не каскадом БД**:
