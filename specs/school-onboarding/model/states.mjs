...
export const reversals = [
  ...
  ['зарегистрировать сотрудника',  'отозвать активацию либо удалить профиль', ''],
  ...
  ['отозвать активацию сотрудника','вернуть доступ сотруднику', ''],
  ...
  ['удалить профиль сотрудника',   '', 'необратимо по построению (AR-212): профиль стирается физически — учётка, креды, членство, сессии, токены входа и карточка; записи школы (отметки, уроки, колонки журнала, аудит) остаются историческими ссылками. Обратимая операция на той же карточке другая — отзыв активации'],
...
// Две разрушающие операции над сотрудником (AR-212, вытесняет правило подмены
// AR-89): выбирает их человек, а не сервер, и различаются они объёмом потери
// данных. Сервер держит одно — школа не теряет последнего активного модератора
// ни одной из них. История сотрудника на выбор больше не влияет: удаляется и
// тот, кто уже выставил отметки, — отметки остаются записью школы.
export const staffRemoval = (person, school, op = 'delete') => {
  ...
  if (op === 'revoke')
    return { action: 'revoke', keepsMarks: true, keepsProfile: true, unbinds: true, staleSchedule: true, reversal: 'вернуть доступ' };
  return { action: 'delete', keepsMarks: true, keepsProfile: false, unbinds: true, staleSchedule: true, reversal: null };
};
